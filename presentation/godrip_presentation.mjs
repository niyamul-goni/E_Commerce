...
pptx.defineSlideMaster({
  ...
  objects: [],
  // Page numbers are drawn explicitly by base() (and the title slide) so we do
  // NOT set a master slideNumber here — otherwise every content slide renders
...
// ---------------------------------------------------------------------------
{
  ...
  connector(slide, 8.30, 3.60, 12.85, 3.60, { color: C.gold, width: 1.2 });
  addText(slide, "PRODUCT  /  VARIANT  /  STOCK  /  ORDER", 8.42, 3.66, 4.35, 0.24, {
...
// ---------------------------------------------------------------------------
{
  ...
  // Connectors first.
  connector(slide, 3.52, 3.48, 4.05, 3.48, { color: C.gold, width: 2, endArrowType: "triangle" });
...
// ---------------------------------------------------------------------------
{
  ...
  layers.forEach(([name, sub, accent], i) => {
    ...
    addText(slide, name, xs[i] + 0.10, 2.90, widths[i] - 0.20, 0.52, {
      fontFace: F.display, fontSize: 13.5, color: C.ivory, bold: true, align: "center", valign: "mid",
    });
...
// ---------------------------------------------------------------------------
{
  ...
  // Connectors first (drawn beneath the boxes), then the entities.
  // --- Main spine: PRODUCTS -> VARIANTS -> INVENTORY -> MOVEMENTS (centre y 3.87)
  connector(slide, 5.30, 3.87, 6.05, 3.87, { color: C.gold, width: 2.0, endArrowType: "triangle" });
  connector(slide, 8.00, 3.87, 8.80, 3.87, { color: C.green, width: 2.0, endArrowType: "triangle" });
  connector(slide, 10.65, 3.87, 11.05, 3.87, { color: C.green, width: 1.6, endArrowType: "triangle" });

  // --- Left source tables feed PRODUCTS (target left edge x 3.25)
  connector(slide, 2.30, 2.37, 3.25, 3.58, { color: C.gold, width: 1.0 });
  connector(slide, 2.30, 3.25, 3.25, 3.74, { color: C.gold, width: 1.0 });
  connector(slide, 1.52, 4.48, 1.52, 4.66, { color: C.gold, width: 1.0 });        // categories -> subcategories
  connector(slide, 2.30, 5.01, 3.25, 4.12, { color: C.gold, width: 1.0 });        // subcategories -> products
  connector(slide, 2.30, 5.89, 3.25, 4.26, { color: C.blue, width: 0.9 });        // genders -> products

  // --- Top descriptive tables
  connector(slide, 4.15, 2.63, 4.15, 3.35, { color: C.gold, width: 1.0 });        // product_images -> products
  connector(slide, 5.85, 2.63, 4.85, 3.35, { color: C.gold, width: 1.0 });        // product_specifications -> products
  connector(slide, 9.30, 2.29, 9.55, 2.29, { color: C.violet, width: 1.0 });      // collections <-> product_collections
  connector(slide, 9.55, 2.45, 5.30, 3.42, { color: C.violet, width: 0.9 });      // product_collections -> products (M:M)

  // --- Bottom lookups / warehouse
  connector(slide, 7.02, 5.05, 7.02, 4.40, { color: C.blue, width: 1.0 });        // color/size/material -> variants
  connector(slide, 9.72, 5.05, 9.72, 4.40, { color: C.green, width: 1.0 });       // warehouses -> inventory

  // Left source stack.
  entity(slide, { x: 0.75, y: 2.02, w: 1.55, name: "brands", keys: "PK id\nUK name, slug", accent: C.gold });
  entity(slide, { x: 0.75, y: 2.90, w: 1.55, name: "suppliers", keys: "PK id\nUK contact_email", accent: C.gold });
  entity(slide, { x: 0.75, y: 3.78, w: 1.55, name: "categories", keys: "PK id\nUK slug", accent: C.gold });
  entity(slide, { x: 0.75, y: 4.66, w: 1.55, name: "subcategories", keys: "PK id\nFK category_id", accent: C.gold });
  entity(slide, { x: 0.75, y: 5.54, w: 1.55, name: "genders", keys: "PK id\nUK name", accent: C.blue });

  // Top descriptive tables.
  entity(slide, { x: 3.25, y: 1.95, w: 1.85, name: "product_images", keys: "PK id\nFK product_id", accent: C.gold });
  entity(slide, { x: 5.35, y: 1.95, w: 2.05, name: "product_specifications", keys: "PK id\nUK product_id + key", accent: C.gold, fontSize: 10.6 });
  entity(slide, { x: 7.75, y: 1.95, w: 1.55, name: "collections", keys: "PK id\nFK season_id", accent: C.violet });
  entity(slide, { x: 9.55, y: 1.95, w: 1.95, name: "product_collections", keys: "PK product_id + collection_id", accent: C.violet, fontSize: 9.5 });

  // Spine hubs.
  entity(slide, {
    x: 3.25, y: 3.35, w: 2.05, h: 1.05, name: "products",
    keys: "PK id\nFK brand • supplier\nsubcategory • collection • gender",
    accent: C.gold, strong: true, fontSize: 12.5,
  });
  entity(slide, {
    x: 6.05, y: 3.35, w: 1.95, h: 1.05, name: "product_variants",
    keys: "PK id\nFK product • color • size • material\nUK sku • barcode",
    accent: C.gold, strong: true, fontSize: 11.5,
  });
  entity(slide, {
    x: 8.80, y: 3.35, w: 1.85, h: 1.05, name: "inventory",
    keys: "PK id\nUK variant + warehouse\ngenerated available_stock",
    accent: C.green, strong: true, fontSize: 11.2,
  });
  entity(slide, { x: 11.05, y: 3.52, w: 1.50, h: 0.72, name: "movements", keys: "FK inventory_id\nappend-only ledger", accent: C.green, fontSize: 10.0 });

  // Bottom lookups / warehouse.
  entity(slide, { x: 6.05, y: 5.05, w: 1.95, name: "color • size • material", keys: "lookup PKs\n1:M → variants", accent: C.blue, fontSize: 10.6 });
  entity(slide, { x: 8.80, y: 5.05, w: 1.85, name: "warehouses", keys: "PK id\nUK code", accent: C.green });

  cardinality(slide, "1:M", 5.55, 3.60, C.gold);
  cardinality(slide, "1:M", 8.28, 3.60, C.green);
  cardinality(slide, "1:M", 10.66, 3.60, C.green);
  cardinality(slide, "M:M", 7.30, 2.72, C.violet);
...
// ---------------------------------------------------------------------------
{
  ...
  // Connectors first (drawn beneath the boxes), then the entities.
  // --- Main spine: CUSTOMERS -> ORDERS -> ORDER_ITEMS -> VARIANTS (centre y 3.72)
  connector(slide, 2.42, 3.72, 3.55, 3.72, { color: C.gold, width: 2.0, endArrowType: "triangle" });
  connector(slide, 5.50, 3.72, 6.10, 3.72, { color: C.gold, width: 2.0, endArrowType: "triangle" });
  connector(slide, 8.05, 3.72, 8.70, 3.72, { color: C.gold, width: 2.0, endArrowType: "triangle" });

  // --- Customer satellites feed CUSTOMERS
  connector(slide, 1.57, 2.67, 1.57, 3.30, { color: C.blue, width: 1.0 });        // profiles -> customers
  connector(slide, 1.57, 5.35, 1.57, 4.15, { color: C.blue, width: 1.0 });        // addresses -> customers
  connector(slide, 2.55, 2.66, 2.42, 3.35, { color: C.blue, width: 0.9 });        // cart -> customers
  connector(slide, 2.55, 4.71, 2.42, 4.05, { color: C.blue, width: 0.9 });        // wishlist -> customers

  // --- Orders -> fulfilment top band (1:1 links)
  connector(slide, 4.60, 3.25, 4.95, 2.67, { color: C.gold, width: 0.9 });        // payments
  connector(slide, 5.05, 3.25, 6.60, 2.67, { color: C.green, width: 0.9 });       // shipments
  connector(slide, 5.35, 3.28, 8.35, 2.67, { color: C.gold, width: 0.9 });        // invoices

  // --- Orders -> bottom band
  connector(slide, 4.35, 4.20, 4.45, 5.35, { color: C.gold, width: 0.9 });        // status_history
  connector(slide, 4.95, 4.20, 6.35, 5.35, { color: C.gold, width: 0.9 });        // coupon_usages

  // --- Returns / refunds (routed through the clean band above the spine)
  connector(slide, 4.90, 3.25, 4.90, 2.95, { color: C.coral, width: 1.0 });
  connector(slide, 4.90, 2.95, 10.65, 2.95, { color: C.coral, width: 1.0, endArrowType: "triangle" });
  connector(slide, 11.57, 3.42, 11.57, 3.70, { color: C.coral, width: 1.0, endArrowType: "triangle" }); // return -> refund

  // --- Reviews (variant -> reviews -> media)
  connector(slide, 9.57, 4.15, 9.57, 5.35, { color: C.coral, width: 1.0 });
  connector(slide, 10.40, 5.71, 10.55, 5.71, { color: C.coral, width: 0.9 });

  // Spine hubs.
  entity(slide, {
    x: 0.72, y: 3.30, w: 1.70, h: 0.85, name: "customers",
    keys: "PK id\nUK email", accent: C.blue, strong: true, fontSize: 12.5,
  });
  entity(slide, {
    x: 3.55, y: 3.25, w: 1.95, h: 0.95, name: "orders",
    keys: "PK id • UK order_number\nFK customer • addresses\nship method • coupon",
    accent: C.gold, strong: true, fontSize: 11.5,
  });
  entity(slide, {
    x: 6.10, y: 3.25, w: 1.95, h: 0.95, name: "order_items",
    keys: "PK id\nFK order_id • variant_id\ngenerated line_total",
    accent: C.gold, strong: true, fontSize: 11.8,
  });
  entity(slide, {
    x: 8.70, y: 3.30, w: 1.75, h: 0.85, name: "product_variants",
    keys: "PK id\nUK sku • barcode",
    accent: C.gold, strong: true, fontSize: 11.5,
  });

  // Customer satellites.
  entity(slide, { x: 0.72, y: 1.95, w: 1.70, name: "customer_profiles", keys: "PK/FK customer_id\nUK phone", accent: C.blue, fontSize: 10.3 });
  entity(slide, { x: 0.72, y: 5.35, w: 1.70, name: "addresses", keys: "PK id\nFK customer_id", accent: C.blue });
  entity(slide, { x: 2.55, y: 2.30, w: 1.60, name: "cart + items", keys: "customer 1:1 cart\nvariant 1:M items", accent: C.blue, fontSize: 10.4 });
  entity(slide, { x: 2.55, y: 4.35, w: 1.60, name: "wishlist + items", keys: "customer 1:1 list\nvariant 1:M items", accent: C.blue, fontSize: 10.4 });

  // Order fulfilment top band.
  entity(slide, { x: 4.35, y: 1.95, w: 1.70, name: "payments", keys: "FK/UK order_id\nUK transaction_ref", accent: C.gold, fontSize: 10.6 });
  entity(slide, { x: 6.20, y: 1.95, w: 1.70, name: "shipments", keys: "FK/UK order_id\nUK tracking_number", accent: C.green, fontSize: 10.6 });
  entity(slide, { x: 8.05, y: 1.95, w: 1.70, name: "invoices", keys: "FK/UK order_id\nUK invoice_number", accent: C.gold, fontSize: 10.6 });

  // Order bottom band.
  entity(slide, { x: 3.55, y: 5.35, w: 1.95, name: "status_history", keys: "FK order_id\nFK changed_by admin", accent: C.gold });
  entity(slide, { x: 5.75, y: 5.35, w: 1.95, name: "coupon_usages", keys: "FK coupon • customer • order", accent: C.gold, fontSize: 10.6 });

  // Returns / refunds / feedback.
  entity(slide, { x: 10.65, y: 2.70, w: 1.85, name: "return_requests", keys: "FK order • customer\nFK approved_by admin", accent: C.coral, fontSize: 10.2 });
  entity(slide, { x: 10.65, y: 3.70, w: 1.85, h: 0.60, name: "refunds", keys: "FK/UK return_id", accent: C.coral, fontSize: 10.2 });
  entity(slide, { x: 8.60, y: 5.35, w: 1.80, name: "reviews", keys: "FK customer • variant • order", accent: C.coral, fontSize: 10.6 });
  entity(slide, { x: 10.55, y: 5.35, w: 1.95, name: "review media + reply", keys: "images 1:M\nreply 1:1 + admin", accent: C.coral, fontSize: 9.8 });

  cardinality(slide, "1:M", 2.88, 3.48, C.gold);
  cardinality(slide, "1:M", 5.62, 3.48, C.gold);
  cardinality(slide, "M:1", 8.10, 3.48, C.gold);
  cardinality(slide, "1:M", 9.62, 4.60, C.coral);
...
// ---------------------------------------------------------------------------
{
  ...
  groups.forEach(([title, body, accent], i) => {
    ...
    const y = 1.98 + row * 1.62;
    rect(slide, x, y, 3.78, 1.42, { fill: C.surface, line: C.line });
    sectionLabel(slide, title, x + 0.20, y + 0.18, accent);
    const lines = body.split("\n");
    if (lines.length > 6) {
      // Split long lists (e.g. CUSTOMER / 8) into two readable columns.
      const half = Math.ceil(lines.length / 2);
      addText(slide, lines.slice(0, half).join("\n"), x + 0.20, y + 0.52, 1.66, 0.82, {
        fontFace: F.mono, fontSize: 10.0, color: C.ivory,
      });
      addText(slide, lines.slice(half).join("\n"), x + 1.84, y + 0.52, 1.86, 0.82, {
        fontFace: F.mono, fontSize: 10.0, color: C.ivory,
      });
    } else {
      addText(slide, body, x + 0.20, y + 0.52, 3.35, 0.82, {
